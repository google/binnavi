...
public class ARMUsat16TranslatorTest {
  ...
  private final ARMUsat16Translator translator = new ARMUsat16Translator();
  ...
  public void testSimpleRegister() throws InternalTranslationException, InterpreterException {
    ...
    final IInstruction instruction = new MockInstruction("USAT16", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0x01ff0000L), interpreter.getVariableValue("R0"));
