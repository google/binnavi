...
import com.google.security.zynamics.reil.translators.ppc.RlwimiTranslator;
...
public class RlwimiTranslatorTest {
  ...
  private final RlwimiTranslator translator = new RlwimiTranslator();
  ...
  public void testSimple() throws InternalTranslationException, InterpreterException {
    interpreter.setRegister("%r0", BigInteger.valueOf(0x3L), OperandSize.BYTE,
        ReilRegisterStatus.DEFINED);
    interpreter.setRegister("%r18", BigInteger.valueOf(0x90003000L), OperandSize.DWORD,
        ReilRegisterStatus.DEFINED);
    ...
    operandTree5.root.m_children
        .add(new MockOperandTreeNode(ExpressionType.IMMEDIATE_INTEGER, "29"));


    final List<MockOperandTree> operands =
        Lists.newArrayList(operandTree1, operandTree2, operandTree3, operandTree4, operandTree5);

    final IInstruction instruction = new MockInstruction("rlwimi", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0x4000C003L), interpreter.getVariableValue("%r0"));
    assertEquals(BigInteger.valueOf(0x90003000L), interpreter.getVariableValue("%r18"));
