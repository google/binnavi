...
import com.google.security.zynamics.reil.translators.x64.StosbTranslator;
...
public class StosbTranslatorTest {
  private final ReilInterpreter interpreter = new ReilInterpreter(Endianness.LITTLE_ENDIAN,
      new CpuPolicyX64(), new EmptyInterpreterPolicy());

  private final StandardEnvironment environment = new StandardEnvironment();

  private final StosbTranslator translator = new StosbTranslator();
  ...
  public void testDFCleared() throws InternalTranslationException, InterpreterException {
    ...
    final MockInstruction instruction =
        new MockInstruction("stosb", new ArrayList<MockOperandTree>());
    ...
    assertEquals(BigInteger.valueOf(0x1001), interpreter.getVariableValue("rdi"));

    assertEquals(BigInteger.valueOf(1L), BigInteger.valueOf(interpreter.getMemorySize()));
    assertEquals(BigInteger.valueOf(0x78), interpreter.getMemory().load(0x1000, 1));
  }
  ...
  public void testDFSet() throws InternalTranslationException, InterpreterException {
    ...
    assertEquals(BigInteger.valueOf(0x12345678), interpreter.getVariableValue("rax"));
    assertEquals(BigInteger.valueOf(0x0FFF), interpreter.getVariableValue("rdi"));
