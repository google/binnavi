...
public class THUMBRorTranslatorTest {
  ...
  private final THUMBRorTranslator translator = new THUMBRorTranslator();
  ...
  public void testSimple() throws InternalTranslationException, InterpreterException {
    ...
    final IInstruction instruction = new MockInstruction("RORS", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0x22000000L), interpreter.getVariableValue("R0"));
