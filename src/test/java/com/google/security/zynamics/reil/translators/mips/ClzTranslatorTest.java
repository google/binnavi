...
public class ClzTranslatorTest {
  private final ReilInterpreter interpreter = new ReilInterpreter(Endianness.LITTLE_ENDIAN,
      new CpuPolicyMips(), new EmptyInterpreterPolicy());

  private final StandardEnvironment environment = new StandardEnvironment();

  private final ClzTranslator translator = new ClzTranslator();

  private final ArrayList<ReilInstruction> instructions = new ArrayList<ReilInstruction>();

  @Test
  public void testClzOne() throws InternalTranslationException, InterpreterException {
  ...
  @Test
  public void testClzTwo() throws InternalTranslationException, InterpreterException {
    interpreter.setRegister("$t7", BigInteger.valueOf(0x0L), OperandSize.DWORD,
        ReilRegisterStatus.DEFINED);
    interpreter.setRegister("$t6", BigInteger.valueOf(0x0040001CL), OperandSize.DWORD,
        ReilRegisterStatus.DEFINED);
    ...
    assertEquals(3, TestHelpers.filterNativeRegisters(interpreter.getDefinedRegisters()).size());
    assertEquals(BigInteger.valueOf(0x9L), interpreter.getVariableValue("$t7"));
    assertEquals(BigInteger.valueOf(0x0040001CL), interpreter.getVariableValue("$t6"));
    assertEquals(BigInteger.ZERO, BigInteger.valueOf(interpreter.getMemorySize()));
  }
}
