...
public class ARMRsbTranslatorTest {
  ...
  private final ARMRsbTranslator translator = new ARMRsbTranslator();
  ...
  public void testSimpleRegister() throws InternalTranslationException, InterpreterException {
    ...
    final IInstruction instruction = new MockInstruction("RSB", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0xECCCA888L), interpreter.getVariableValue("R0"));
