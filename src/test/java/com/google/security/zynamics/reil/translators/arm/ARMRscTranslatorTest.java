...
public class ARMRscTranslatorTest {
  ...
  private final ARMRscTranslator translator = new ARMRscTranslator();
  ...
  public void testSimpleRegister() throws InternalTranslationException, InterpreterException {
    ...
    final IInstruction instruction = new MockInstruction("RSC", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0xECCCA887L), interpreter.getVariableValue("R0"));
