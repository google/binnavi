...
public class ARMUmullTranslatorTest {
  ...
  private final ARMUmullTranslator translator = new ARMUmullTranslator();
  ...
  public void testSimpleRegister() throws InternalTranslationException, InterpreterException {
    interpreter.setRegister("R0", BigInteger.valueOf(0x42D4L), dw, ReilRegisterStatus.DEFINED);
    interpreter.setRegister("R1", BigInteger.valueOf(0x40000010L), dw, ReilRegisterStatus.DEFINED);
    interpreter.setRegister("R2", BigInteger.valueOf(0x77654321L), dw, ReilRegisterStatus.DEFINED);
    interpreter.setRegister("R3", BigInteger.valueOf(0xFF00FF00L), dw, ReilRegisterStatus.DEFINED);
    ...
    final List<MockOperandTree> operands =
        Lists.newArrayList(operandTree1, operandTree2, operandTree3, operandTree4);

    final IInstruction instruction = new MockInstruction("UMULL", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0xBCDDDF00L), interpreter.getVariableValue("R0"));
    assertEquals(BigInteger.valueOf(0x76EE54CBL), interpreter.getVariableValue("R1"));
    assertEquals(BigInteger.valueOf(0x77654321L), interpreter.getVariableValue("R2"));
    assertEquals(BigInteger.valueOf(0xFF00FF00L), interpreter.getVariableValue("R3"));
