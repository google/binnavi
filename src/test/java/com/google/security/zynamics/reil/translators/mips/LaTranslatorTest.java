...
public class LaTranslatorTest {
  private final ReilInterpreter interpreter = new ReilInterpreter(Endianness.LITTLE_ENDIAN,
      new CpuPolicyMips(), new EmptyInterpreterPolicy());

  private final StandardEnvironment environment = new StandardEnvironment();

  private final LaTranslator translator = new LaTranslator();
  ...
  public void testLaOne() throws InternalTranslationException, InterpreterException {
    ...
    operandTree2.root.m_children.add(new MockOperandTreeNode(ExpressionType.IMMEDIATE_INTEGER,
        String.valueOf(0x00123)));

    final List<MockOperandTree> operands = Lists.newArrayList(operandTree1, operandTree2);

    final IInstruction instruction = new MockInstruction("la", operands);
    ...
    assertEquals(BigInteger.valueOf(0x00123L), interpreter.getVariableValue("$v1"));
