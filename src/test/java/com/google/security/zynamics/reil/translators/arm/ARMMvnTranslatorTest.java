...
public class ARMMvnTranslatorTest {
  ...
  private final ARMMvnTranslator translator = new ARMMvnTranslator();
  ...
  public void testSimpleRegister() throws InternalTranslationException, InterpreterException {
    ...
    final IInstruction instruction = new MockInstruction("MVN", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0xEDCBA987L), interpreter.getVariableValue("R0"));
