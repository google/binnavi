...
public class THUMBMovTranslatorTest {
  ...
  private final THUMBMovTranslator translator = new THUMBMovTranslator();
  ...
  public void testSimple() throws InternalTranslationException, InterpreterException {
    ...
    final IInstruction instruction = new MockInstruction("MOVS", operands);

    translator.translate(environment, instruction, instructions);

    interpreter.interpret(TestHelpers.createMapping(instructions), BigInteger.valueOf(0x100L));

    assertEquals(BigInteger.valueOf(0x00000022L), interpreter.getVariableValue("R0"));
