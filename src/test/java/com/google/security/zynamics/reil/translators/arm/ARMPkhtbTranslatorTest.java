...
public class ARMPkhtbTranslatorTest {
  ...
  private final ARMPkhtbTranslator translator = new ARMPkhtbTranslator();
  ...
  public void testSimpleASR() throws InternalTranslationException, InterpreterException {
    ...
    interpreter.setRegister("R2", BigInteger.valueOf(0xED950CE6L), dw, ReilRegisterStatus.DEFINED);
    ...
    operandTree3.root.m_children.get(0).m_children.add(new MockOperandTreeNode(
        ExpressionType.REGISTER, "R2"));
    operandTree3.root.m_children.get(0).m_children.add(new MockOperandTreeNode(
        ExpressionType.IMMEDIATE_INTEGER, "7"));

    final List<MockOperandTree> operands =
        Lists.newArrayList(operandTree1, operandTree2, operandTree3);

    final IInstruction instruction = new MockInstruction("PKHTB", operands);
    ...
    assertEquals(BigInteger.valueOf(0xED950CE6L), interpreter.getVariableValue("R2"));
    assertEquals(BigInteger.valueOf(0x01FF2A19L), interpreter.getVariableValue("R0"));
