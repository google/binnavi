...
/**
 * Parser responsible for parsing replies to Attach requests.
 */
public final class AttachParser extends AbstractReplyParser<AttachReply> {
  /**
   * Creates a new Attach reply parser.
   *
   * @param clientReader Used to read messages sent by the debug client.
   */
  public AttachParser(final ClientReader clientReader) {
    super(clientReader, DebugCommandType.RESP_ATTACH_SUCCESS);
  }

  @Override
  protected AttachReply parseError(final int packetId) throws IOException {
    final int errorCode = parseInteger();

    return new AttachReply(packetId, errorCode);
  }

  @Override
  public AttachReply parseSuccess(final int packetId, final int argumentCount) {
    return new AttachReply(packetId, 0);
  }
}
