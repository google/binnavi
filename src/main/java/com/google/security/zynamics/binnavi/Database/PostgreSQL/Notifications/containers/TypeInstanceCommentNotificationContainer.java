...
/**
 * Class to store information for {@link TypeInstance type instance} comment notification.
 */
public class TypeInstanceCommentNotificationContainer extends AbstractCommentNotification {

  private final TypeInstance instance;

  public TypeInstanceCommentNotificationContainer(
      final TypeInstance instance, final CommentOperation operation, final Integer commentId) {
  ...
  void informDelete(CommentManager manager, IComment comment) {
    manager.deleteTypeInstanceCommentInternal(instance, comment);
  }

  @Override
  void informAppend(CommentManager manager) throws CouldntLoadDataException {
    manager.appendTypeInstanceComment(instance, getCommentId());
  }
}
