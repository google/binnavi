...
/**
 * Filters modules according to the number of their views.
 */
public final class CViewFilterExpression extends CNumericFilterExpression<CModuleWrapper> {
  /**
   * Creates a new filter expression object.
   * 
   * @param relation Relation used to filter.
   * @param value Value to compare the view count of the modules to.
   */
  public CViewFilterExpression(final FilterRelation relation, final long value) {
    super(relation, value);
  }

  @Override
  public boolean evaluate(final CModuleWrapper module) {
    return evaluate(module.unwrap().getViewCount());
  }
}
