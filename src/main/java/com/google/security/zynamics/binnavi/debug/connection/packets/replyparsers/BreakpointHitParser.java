...
/**
 * Parser responsible for parsing Breakpoint hit replies.
 */
public final class BreakpointHitParser extends AbstractReplyParser<BreakpointHitReply> {
  /**
   * Creates a new Breakpoint hit reply parser.
   *
   * @param clientReader Used to read messages sent by the debug client.
   */
  public BreakpointHitParser(final ClientReader clientReader) {
    super(clientReader, DebugCommandType.RESP_BP_HIT);
  }

  @Override
  protected BreakpointHitReply parseError(final int packetId) {
    // TODO: There is no proper handling of errors on the side of the
    // client yet.

    throw new IllegalStateException("IE01080: Received invalid reply from the debug client");
  }

  @Override
  public BreakpointHitReply parseSuccess(final int packetId, final int argumentCount)
      throws IOException {
