...
public final class TagTreeManager {
   ...
   */
  // / @endcond
  public TagTreeManager(final ITagTreeManager manager) {
    m_manager = manager;
   ...
   *         manager.
   */
  public void addListener(final ITagTreeManagerListener listener) {
  ...
  public Tag addTag(final Tag parent, final String name) throws CouldntSaveDataException {
    try {
      if (parent == null) {
        return ObjectFinders.getObject(m_manager.addTag(m_root.getNative(), name),
            m_allTags.values());
      } else {
        return ObjectFinders.getObject(m_manager.addTag(parent.getNative(), name),
            m_allTags.values());
      }
  ...
  public Tag insertTag(final Tag parent, final String name) throws CouldntSaveDataException {
    try {
      if (parent == null) {
        return ObjectFinders.getObject(m_manager.insertTag(m_root.getNative(), name),
            m_allTags.values());
      } else {
        return ObjectFinders.getObject(m_manager.insertTag(parent.getNative(), name),
            m_allTags.values());
      }
    } catch (final com.google.security.zynamics.binnavi.Database.Exceptions.CouldntSaveDataException exception) {
      throw new CouldntSaveDataException(exception);
   ...
   *         manager.
   */
  public void removeListener(final ITagTreeManagerListener listener) {
  ...
  private class InternalListener implements
      com.google.security.zynamics.binnavi.Tagging.ITagTreeManagerListener {
    /**
    ...
    @Override
    public void addedTag(final ITagTreeManager manager, final ITreeNode<CTag> tag) {
    ...
    @Override
    public void deletedTag(final ITagTreeManager manager, final ITreeNode<CTag> parent,
    ...
    @Override
    public void deletedTagSubtree(final ITagTreeManager manager, final ITreeNode<CTag> parent,
        final ITreeNode<CTag> tag) {
      final Tag deletedTag = m_allTags.get(tag);
    ...
    @Override
    public void insertedTag(final ITagTreeManager tagManager, final ITreeNode<CTag> parent,
        final ITreeNode<CTag> tag) {
      final Tag parentTag = m_allTags.get(tag.getParent());
