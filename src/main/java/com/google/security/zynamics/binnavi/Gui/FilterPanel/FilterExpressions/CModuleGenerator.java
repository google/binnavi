...
/**
 * Takes a filter string and converts it into a filter expression for filtering address spaces
 * according to the number of their modules.
 */
public final class CModuleGenerator implements IPredicateGenerator<CAddressSpaceWrapper> {
  /**
   * Regular expression that matches valid filter strings.
   */
  private static final String RULE_REGEX = "\\s*modules\\s*(==|!=|<|>|<=|>=|<>)\\s*(\\d+)\\s*";
