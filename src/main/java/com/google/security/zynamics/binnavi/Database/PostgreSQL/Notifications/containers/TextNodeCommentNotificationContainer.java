...
/**
 * Class to store information for {@link INaviTextNode text node} comment notification.
 */
public class TextNodeCommentNotificationContainer extends AbstractCommentNotification {

  private final INaviTextNode node;

  public TextNodeCommentNotificationContainer(
      final INaviTextNode node, final CommentOperation operation, final Integer commentId) {
    super(operation, commentId);
    this.node = Preconditions.checkNotNull(node, "Error: Node argument can not be null.");
  }

  public INaviTextNode getNode() {
    return node;
  }

  @Override
  void informDelete(CommentManager manager, IComment comment) {
    manager.deleteTextNodeCommentInternal(node, comment);
  }

  @Override
  void informAppend(CommentManager manager) throws CouldntLoadDataException {
    manager.appendTextNodeComment(node, getCommentId());
  }
}
