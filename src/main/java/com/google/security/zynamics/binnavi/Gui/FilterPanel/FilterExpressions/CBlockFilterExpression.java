...
/**
 * Filters views according to the number of their nodes.
 */
public final class CBlockFilterExpression extends CNumericFilterExpression<CViewWrapper> {
  /**
   * Creates a new filter expression object.
   * 
   * @param relation Relation used to filter.
   * @param value Value to compare the node count of the views to.
   */
  public CBlockFilterExpression(final FilterRelation relation, final long value) {
    super(relation, value);
  }

  @Override
  public boolean evaluate(final CViewWrapper viewWrapper) {
    return evaluate(viewWrapper.unwrap().getNodeCount());
  }
}
