...
/**
 * Generator that generates code for the ECXZero condition.
 */
public class EcxZeroGenerator implements IConditionGenerator {

  /**
   * Generates code for the ECXZero condition.
   * 
  ...
  public Pair<OperandSize, String> generate(final ITranslationEnvironment environment,
    ...
    final String isolatedECx = environment.getNextVariableString();
    final String ecxZero = environment.getNextVariableString();

    instructions.add(ReilHelpers.createAnd(offset, OperandSize.QWORD, "rcx", OperandSize.DWORD,
            "4294967295", OperandSize.DWORD, isolatedECx));
    instructions.add(ReilHelpers.createBisz(offset + 1, OperandSize.DWORD, isolatedECx,
        OperandSize.BYTE, ecxZero));
