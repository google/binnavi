...
public class THUMBLsrTranslator extends ARMBaseTranslator {
  @Override
  protected void translateCore(final ITranslationEnvironment environment,
    ...
    if (instruction.getOperands().size() == 3) {
      if (instruction.getMnemonic().endsWith(".W")) {
        result = AddressingModeOneGenerator.lsrRegister(baseOffset, environment, instructions,
            sourceRegister2, sourceImmediate);
      } else {
        result = AddressingModeOneGenerator.lsrImmediate(baseOffset, environment, instructions,
            sourceRegister2, sourceImmediate);
      }
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    translateAll(environment, instruction, "LSR", instructions);
  }
}
