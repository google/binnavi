...
public class THUMBPopTranslator extends ARMBaseTranslator {
  @Override
  protected void translateCore(final ITranslationEnvironment environment,
    ...
    instructions.add(ReilHelpers.createAdd(baseOffset++, dw,
        String.valueOf(4 * registerListLength), dw, tmpAddress, dw, ("SP")));

    for (int i = 0; i < registerListLength; i++) {
      if ((instruction.getOperands().get(0).getRootNode().getChildren().get(0).getChildren().get(i)
          .getValue()).equalsIgnoreCase("PC")) {
        ...
        instructions.add(ReilHelpers.createAnd(baseOffset++, dw, tmpValue, dw,
            String.valueOf(0xFFFFFFFEL), dw, jmpTarget));
        instructions
            .add(ReilHelpers.createJcc(baseOffset++, dw, String.valueOf(1L), dw, jmpTarget));
      } else {
        instructions.add(ReilHelpers.createLdm(
            ...
            (instruction.getOperands().get(0).getRootNode().getChildren().get(0).getChildren()
                .get(i).getValue())));
        instructions.add(ReilHelpers.createAdd(baseOffset++, dw, tmpAddress, bt,
            String.valueOf(4L), dw, tmpAddress));
      }
    }
  }
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    translateAll(environment, instruction, "POP", instructions);
  }
}
