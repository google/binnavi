...
public class CmplwTranslator implements IInstructionTranslator {
  @Override
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (instruction.getOperands().size() == 2) {
      CmpGenerator.generate(instruction.getAddress().toLong() * 0x100, environment, instruction,
          instructions, "cmplw", "CR0", registerOperand1.getValue(), registerOperand2.getValue(),
          true);
    } else {
      CmpGenerator.generate(instruction.getAddress().toLong() * 0x100, environment, instruction,
          instructions, "cmplw", conditionRegister.getValue(), registerOperand1.getValue(),
