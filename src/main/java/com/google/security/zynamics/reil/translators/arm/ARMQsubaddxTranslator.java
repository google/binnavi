...
public class ARMQsubaddxTranslator extends ARMBaseTranslator {
  @Override
    ...
    new Processor() {
      ...
      protected String[] process(long baseOffset, final String[] firstTwo, final String[] secondTwo) {
        ...
        instructions.add(ReilHelpers.createSub(baseOffset + instructions.size(), dw, firstTwo[1],
            dw, secondTwo[0], dw, diff1));

        // Do the Sat
        Helpers.signedSat(baseOffset + instructions.size(), environment, instruction, instructions,
            dw, firstTwo[0], dw, secondTwo[1], dw, sum1, "ADD", sum1Sat, 16L, "");
        Helpers.signedSat(baseOffset + instructions.size(), environment, instruction, instructions,
            dw, firstTwo[1], dw, secondTwo[0], dw, diff1, "SUB", diff1Sat, 16L, "");
   ...
   * Operation:
   * 
   * if ConditionPassed(cond) then Rd[15:0] = SignedSat(Rn[15:0] + Rm[31:16], 16) Rd[31:16] =
   * SignedSat(Rn[31:16] - Rm[15:0], 16)
   */

  @Override
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
      final List<ReilInstruction> instructions) throws InternalTranslationException {
    if (instruction.getMnemonic().startsWith("QSAX")) {
      TranslationHelpers.checkTranslationArguments(environment, instruction, instructions, "QSAX");
      translateAll(environment, instruction, "QSAX", instructions);
    } else {
      TranslationHelpers.checkTranslationArguments(environment, instruction, instructions,
          "QSUBADDX");
      translateAll(environment, instruction, "QSUBADDX", instructions);
    }
  }
}
