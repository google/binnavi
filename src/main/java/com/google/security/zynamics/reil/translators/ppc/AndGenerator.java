...
public class AndGenerator {
  public static void generate(long baseOffset, final ITranslationEnvironment environment,
      ...
      final boolean setCr, final boolean isNand, final boolean withComplement)
    ...
    // rR = ! ( rA & rB )
    if (isNand) {
      instructions.add(ReilHelpers.createAnd(baseOffset++, OperandSize.DWORD, firstOperand,
          OperandSize.DWORD, secondOperand, OperandSize.DWORD, nandVar));
      instructions.add(ReilHelpers.createXor(baseOffset++, OperandSize.DWORD, nandVar,
