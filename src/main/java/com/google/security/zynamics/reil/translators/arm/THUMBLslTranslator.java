...
public class THUMBLslTranslator extends ARMBaseTranslator {
  @Override
  protected void translateCore(final ITranslationEnvironment environment,
    ...
    final String sourceImmediate =
        instruction.getOperands().size() == 3 ? (registerOperand3.getValue()) : null;
    ...
    if (instruction.getOperands().size() == 3) {
      if (instruction.getMnemonic().endsWith(".W")) {
        result = AddressingModeOneGenerator.lslRegister(baseOffset, environment, instructions,
            sourceRegister2, sourceImmediate);
      } else {
        result = AddressingModeOneGenerator.lslImmediate(baseOffset, environment, instructions,
            sourceRegister2, sourceImmediate);
      }
    ...
    if (instruction.getMnemonic().contains("LSLS")) {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    TranslationHelpers.checkTranslationArguments(environment, instruction, instructions, "LSL");
    translateAll(environment, instruction, "LSL", instructions);
  }
}
