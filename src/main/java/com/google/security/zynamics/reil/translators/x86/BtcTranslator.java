...
/**
 * Translates BTC instructions to REIL code.
 */
public class BtcTranslator implements IInstructionTranslator {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (instruction.getOperands().size() != 2) {
      throw new InternalTranslationException(
          "Error: Argument instruction is not a btc instruction (invalid number of operands)");
    }
    ...
    instructions.add(ReilHelpers.createXor(offset++, targetResult.getSize(),
        targetResult.getRegister(), targetResult.getSize(), shiftedIndex, targetResult.getSize(),
        andedResult));
