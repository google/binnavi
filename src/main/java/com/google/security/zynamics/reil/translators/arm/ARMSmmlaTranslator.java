...
public class ARMSmmlaTranslator extends ARMBaseTranslator {
  @Override
  protected void translateCore(final ITranslationEnvironment environment,
    ...
    instructions.add(ReilHelpers.createBsh(
        baseOffset++, dw, sourceRegister3, wd, String.valueOf(32), qw, tmpVar1));

    if (instruction.getMnemonic().contains("R")) {
      instructions.add(ReilHelpers.createAdd(
          baseOffset++, qw, value, dw, String.valueOf(0x80000000L), qw, tmpVar2));
    ...
    instructions.add(
        ReilHelpers.createBsh(baseOffset++, qw, tmpVar3, wd, String.valueOf(-32), dw, tmpVar4));
    instructions.add(ReilHelpers.createAnd(
        baseOffset++, dw, tmpVar4, dw, String.valueOf(0xFFFFFFFFL), dw, targetRegister));
  }
   ...
   *  Operation:
   *
   *  if ConditionPassed(cond) then value = Rm * Rs // Signed multiplication if R == 1 then Rd =
   * ((Rn<<32) + value + 0x80000000)[63:32] else Rd = ((Rn<<32) + value)[63:32]
   *
   */
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    TranslationHelpers.checkTranslationArguments(environment, instruction, instructions, "SMMLA");
    translateAll(environment, instruction, "SMMLA", instructions);
  }
}
