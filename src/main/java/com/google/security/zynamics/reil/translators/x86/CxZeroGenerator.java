...
/**
 * Generator that generates code for the CXZero condition.
 */
public class CxZeroGenerator implements IConditionGenerator {

  /**
   * Generates code for the CXZero condition.
   * 
  ...
  public Pair<OperandSize, String> generate(final ITranslationEnvironment environment,
    ...
    instructions.add(ReilHelpers.createAnd(offset, OperandSize.DWORD, "ecx", OperandSize.WORD,
        "65535", OperandSize.WORD, isolatedCx));
    instructions.add(ReilHelpers.createBisz(offset + 1, OperandSize.WORD, isolatedCx,
        OperandSize.BYTE, cxZero));
