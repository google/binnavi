...
public class ARMSaddsubxTranslator extends ARMBaseTranslator {
  ...
  protected void translateCore(final ITranslationEnvironment environment,
    ...
    new Processor() {
      ...
      protected String[] process(final long offset, final String[] firstTwo,
        ...
        instructions.add(ReilHelpers.createBsh(baseOffset + instructions.size(), dw, tmpVar1, wd,
            String.valueOf(-31), bt, "CPSR_GE_2"));
        instructions.add(ReilHelpers.createStr(baseOffset + instructions.size(), bt, "CPSR_GE_2",
            bt, "CPSR_GE_3"));

        // GE[1:0] = if diff >= 0 then 0b11 else 0
        instructions.add(ReilHelpers.createXor(baseOffset + instructions.size(), dw, diff1, dw,
            String.valueOf(0xFFFFFFFFL), dw, tmpVar2));
        instructions.add(ReilHelpers.createBsh(baseOffset + instructions.size(), dw, tmpVar2, wd,
            String.valueOf(-31), bt, "CPSR_GE_0"));
   ...
   * Operation:
   * 
   * if ConditionPassed(cond) then sum = Rn[31:16] + Rm[15:0] // Signed addition
   * Rd[31:16] = sum[15:0] GE[3:2] = if sum >= 0 then 0b11 else 0 diff =
   * Rn[15:0] - Rm[31:16] Signed subtraction Rd[15:0] = diff[15:0] GE[1:0] = if
   * diff >= 0 then 0b11 else 0
   */
