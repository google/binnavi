...
public class CmplwiTranslator implements IInstructionTranslator {
  @Override
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
      final List<ReilInstruction> instructions) throws InternalTranslationException {
    TranslationHelpers.checkTranslationArguments(environment, instruction, instructions, "cmplwi");
    ...
    if (instruction.getOperands().size() == 2) {
      CmpGenerator.generate(instruction.getAddress().toLong() * 0x100, environment, instruction,
          instructions, "cmplwi", "CR0", registerOperand1.getValue(), literalOperand1.getValue(),
          true);
    } else {
      CmpGenerator.generate(instruction.getAddress().toLong() * 0x100, environment, instruction,
          instructions, "cmplwi", conditionRegister.getValue(), registerOperand1.getValue(),
          literalOperand1.getValue(), true);
    }
  }
}
