...
/**
 * Translates NOT instructions to REIL code.
 */
public class NotTranslator implements IInstructionTranslator {
  /**
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (instruction.getOperands().size() != 1) {
      throw new InternalTranslationException(
          "Error: Argument instruction is not an not instruction (invalid number of operands)");
    }
    ...
    instructions.add(ReilHelpers.createXor(offset, size, operandRegister, size, truncateMask, size,
        xorResult));

    // Write the flipped value back to the operand
    Helpers.writeBack(environment, offset + 1, operand, xorResult, size, result.getAddress(),
        result.getType(), instructions);
  }
}
