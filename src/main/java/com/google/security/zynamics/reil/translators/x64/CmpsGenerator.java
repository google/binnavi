...
public class CmpsGenerator implements IStringInstructionGenerator {
  ...
  public void generate(final ITranslationEnvironment environment, final long baseOffset,
    ...
    instructions.add(ReilHelpers.createLdm(offset, archSize, "rsi", operandSize, firstVar));
    instructions.add(ReilHelpers.createLdm(offset + 1, archSize, "rdi", operandSize, secondVar));
    ...
    instructions.add(ReilHelpers.createAdd(offset + 3, archSize, "rsi", archSize, ediChange,
        resultSize, tempEsi1));
    instructions.add(ReilHelpers.createAnd(offset + 4, resultSize, tempEsi1, resultSize,
        truncateMask, archSize, "rsi"));
    instructions.add(ReilHelpers.createAdd(offset + 5, archSize, "rdi", archSize, ediChange,
        resultSize, tempEdi1));
    ...
    instructions.add(ReilHelpers.createJcc(offset + 7, OperandSize.BYTE, "1", OperandSize.ADDRESS,
        jmpGoal2));
    instructions.add(ReilHelpers.createSub(offset + 8, archSize, "rsi", archSize, ediChange,
        resultSize, tempEsi2));
    instructions.add(ReilHelpers.createAnd(offset + 9, resultSize, tempEsi2, resultSize,
        truncateMask, archSize, "rsi"));
    instructions.add(ReilHelpers.createSub(offset + 10, archSize, "rdi", archSize, ediChange,
        resultSize, tempEdi2));
