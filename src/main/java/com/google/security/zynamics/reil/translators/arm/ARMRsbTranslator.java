...
public class ARMRsbTranslator extends ARMBaseTranslator
{
	@Override
	protected void translateCore(final ITranslationEnvironment environment, final IInstruction instruction, final List<ReilInstruction> instructions) throws InternalTranslationException
	{
		...
		instructions.add(ReilHelpers.createSub(baseOffset++, dw, shifterOperand, dw, sourceRegister, dw, tmpVar1));
	...
	{
		TranslationHelpers.checkTranslationArguments(environment, instruction, instructions, "RSB");
		translateAll(environment, instruction, "RSB", instructions);
	}
}
