...
public class ARMSmlawYTranslator extends ARMBaseTranslator {
  @Override
  protected void translateCore(final ITranslationEnvironment environment,
    ...
    Helpers.signedMul(baseOffset, environment, instruction, instructions, dw, operand2, dw,
        sourceRegister2, qw, tmpResult);
    baseOffset = ReilHelpers.nextReilAddress(instruction, instructions);
    instructions.add(ReilHelpers.createBsh(baseOffset++, qw, tmpVar3, wd, String.valueOf(-16L), dw,
        tmpVar4));
    instructions.add(ReilHelpers.createAnd(baseOffset++, dw, tmpVar4, dw,
        String.valueOf(0xFFFFFFFFL), dw, tmpVar5));
    instructions.add(ReilHelpers.createAdd(baseOffset++, dw, sourceRegister4, dw, tmpVar5, qw,
        tmpVar6));

    instructions.add(ReilHelpers.createAnd(baseOffset++, qw, tmpVar6, dw,
        String.valueOf(0xFFFFFFFFL), dw, sourceRegister1));

    // overflow condition
    instructions.add(ReilHelpers.createBsh(baseOffset++, qw, tmpVar6, wd, String.valueOf(-32), dw,
        tmpVar7));
   ...
   * operand2 = SignExtend(Rs[31:16]) Rd = (Rm * operand2)[47:16] + Rn // Signed multiplication if
   * OverflowFrom((Rm * operand2)[47:16] + Rn) then Q Flag = 1
   */
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    translateAll(environment, instruction, "SMLAW", instructions);
  }
}
