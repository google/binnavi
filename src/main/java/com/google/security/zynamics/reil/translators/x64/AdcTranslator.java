...
/**
 * Translates ADC instructions to REIL code.
 *
 * sp
 *
 */
public class AdcTranslator implements IInstructionTranslator {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (instruction.getOperands().size() != 2) {
      throw new InternalTranslationException(
          "Error: Argument instruction is not an adc instruction (invalid number of operands)");
    }
    ...
    final String truncateMask = TranslationHelpers.getAllBitsMask(size).toString();
    ...
    instructions.add(ReilHelpers.createAdd(offset + 2, size, sourceRegister, size, targetRegister,
        resultSize, addResultTemp));
    instructions.add(ReilHelpers.createAdd(offset + 3, resultSize, addResultTemp, OperandSize.BYTE,
        Helpers.CARRY_FLAG, resultSize, addResult));

    // Isolate the MSB of the result and put it into the Sign Flag
    instructions.add(ReilHelpers.createAnd(offset + 4, resultSize, addResult, resultSize, msbMask,
        size, msbResult));
    ...
    instructions.add(ReilHelpers.createXor(offset + 7, size, msbSameBefore, size, msbMask, size,
        msbSameBeforeNeg));
    instructions.add(ReilHelpers.createXor(offset + 8, size, msb1, size, msbResult, size,
        msbChanged));
    ...
    instructions.add(ReilHelpers.createBsh(offset + 10, size, tempOf, size, shiftValue,
        OperandSize.BYTE, Helpers.OVERFLOW_FLAG));

    // Update the Carry Flag
    instructions.add(ReilHelpers.createAnd(offset + 11, resultSize, addResult, resultSize,
        carryMask, resultSize, tempCf));
    instructions.add(ReilHelpers.createBsh(offset + 12, resultSize, tempCf, resultSize, shiftCarry,
        OperandSize.BYTE, Helpers.CARRY_FLAG));

    // Truncate the result to fit into the target
    instructions.add(ReilHelpers.createAnd(offset + 13, resultSize, addResult, resultSize,
        truncateMask, size, truncatedResult));

    // Update the Zero Flag
    instructions.add(ReilHelpers.createBisz(offset + 14, size, truncatedResult, OperandSize.BYTE,
        Helpers.ZERO_FLAG));

    // Write the result of the ADC operation back into the target register
    Helpers.writeBack(environment, offset + 15, targetOperand, truncatedResult, size,
