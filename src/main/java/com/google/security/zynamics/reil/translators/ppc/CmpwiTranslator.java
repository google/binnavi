...
public class CmpwiTranslator implements IInstructionTranslator {
  @Override
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (instruction.getOperands().size() == 2) {
      CmpGenerator.generate(instruction.getAddress().toLong() * 0x100, environment, instruction,
          instructions, "cmpwi", "CR0", registerOperand1.getValue(), literalOperand1.getValue(),
          false);
    } else {
      CmpGenerator.generate(instruction.getAddress().toLong() * 0x100, environment, instruction,
          instructions, "cmpwi", conditionRegister.getValue(), registerOperand1.getValue(),
          literalOperand1.getValue(), false);
    }
  }
}
