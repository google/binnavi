...
/**
 * Generator that generates code for the BelowEqual condition.
 */
public class BelowEqualGenerator implements IConditionGenerator {

  /**
   * Generates code for the BelowEqual condition.
   * 
  ...
  public Pair<OperandSize, String> generate(final ITranslationEnvironment environment,
    ...
    // below or equal: CF or ZF

    final String cfOrZf = environment.getNextVariableString();

    // Find out if either CF or ZF are set
    instructions.add(ReilHelpers.createOr(offset, OperandSize.BYTE, Helpers.CARRY_FLAG,
        OperandSize.BYTE, Helpers.ZERO_FLAG, OperandSize.BYTE, cfOrZf));
