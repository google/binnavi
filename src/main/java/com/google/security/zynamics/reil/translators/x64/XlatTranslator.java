...
public class XlatTranslator implements IInstructionTranslator {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    instructions.add(ReilHelpers.createAnd(offset, OperandSize.QWORD, "rax", OperandSize.DWORD,
        "255", OperandSize.DWORD, isolatedAl));
    instructions.add(ReilHelpers.createAdd(offset + 1, OperandSize.DWORD, isolatedAl,
        OperandSize.QWORD, "rbx", OperandSize.OWORD, address));
    instructions.add(ReilHelpers.createAnd(offset + 2, OperandSize.OWORD, address,
        OperandSize.QWORD, "18446744073709551615", OperandSize.QWORD, truncatedAddress));

    // Load the value from the table
    instructions.add(ReilHelpers.createLdm(offset + 3, OperandSize.QWORD, truncatedAddress,
        OperandSize.BYTE, value));

    // Store the value in AL
    instructions.add(ReilHelpers.createAnd(offset + 4, OperandSize.QWORD, "rax", OperandSize.QWORD,
        "18446744073709551360", OperandSize.QWORD, maskedEax));
    instructions.add(ReilHelpers.createOr(offset + 5, OperandSize.BYTE, value, OperandSize.QWORD,
        maskedEax, OperandSize.QWORD, "rax"));
  }
}
