...
public class MovzxTranslator implements IInstructionTranslator {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (destSize == Helpers.ArchitectureSize) {
      ...
      if(Helpers.is32BitRegister(operands.get(0).toString())) {
        String parentReg = Helpers.getParentRegister(operands.get(0).toString());
        instructions.add(ReilHelpers.createStr(offset, sourceSize, sourceRegister, OperandSize.QWORD, parentReg));
      } else {
