...
#ifndef ARMLittleEndian_HPP
#define ARMLittleEndian_HPP
...
class ARMLittleEndian : public GdbCpu, public BreakpointHandler {
 private:
 ...
 public:
  /**
   * Creates a new ARMLittleEndian CPU object,
   *
   * @param transport The transport object used to communicate with the GDB
   * server.
   */
  ARMLittleEndian(Transport* transport)
      : GdbCpu(transport),
  ...
  // NaviError setRegister(unsigned int tid, unsigned int index, CPUADDRESS
  // address, IEventCallback* cb) const;
